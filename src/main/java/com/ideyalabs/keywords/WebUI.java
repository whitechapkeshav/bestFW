...
import com.ideyalabs.report.ExtentTestManager;
import com.ideyalabs.utils.BrowserInfoUtils;
import com.ideyalabs.utils.DateUtils;
...
public class WebUI {
    ...
    /**
     * Click tab from keyboard.
     */
    public static void clickTab() throws AWTException {
        LogUtils.info("Click TAB using keyboard");
        Robot robot = new Robot();
        robot.keyPress(KeyEvent.VK_TAB);
        robot.keyRelease(KeyEvent.VK_TAB);
        ExtentReportManager.pass("Clicked TAB using keyboard");
        AllureManager.saveTextLog("Clicked TAB using keyboard");
    }
    ...
    public static By getNewLocatorWithColon(By locator,String dynamicText)
    {
        String locatorType = locator.toString().split(": ")[0].split("\\.")[1];
        String newLocatorString=null;
        try{
            if(locator.toString().split(": ")[1]!=null){
                newLocatorString=String.format(locator.toString().split(": ")[1]+": "+locator.toString().split(": ")[2],dynamicText);
            }else
            {newLocatorString = String.format(locator.toString().split(": ")[1],dynamicText);}
        }catch(Exception e){
            e.printStackTrace();
        }
        switch(locatorType)
